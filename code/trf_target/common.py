...
def load_target_config(path: Path) -> dict[str, Any]:
    ...
    if config.get("schema_version") != 1:
        raise TargetTRFError("trf_target config schema_version must equal 1")
    ...
    if not isinstance(source_run_id, str) or not isinstance(source_run_root, str):
    ...
    if Path(source_run_root).name != source_run_id:
        raise TargetTRFError("source_trf.run_root must end with source_trf.run_id")
    if set(source_trf.get("files", {})) != {
        ...
        raise TargetTRFError("source_trf.files does not match the fixed artifact contract")
    if config.get("source_decisions", {}).get("expected_count") != 326:
        raise TargetTRFError("The source decision contract must remain fixed at 326 records")
...
def source_paths(config: dict[str, Any]) -> dict[str, Path]:
    ...
    paths["decisions"] = resolve_project_path(
        PROJECT_ROOT, config["source_decisions"]["path"]
    ).resolve()
    return paths


def snapshot_sources(config: dict[str, Any]) -> dict[str, Any]:
    paths = source_paths(config)
    expected = {
        name: details["sha256"]
        for name, details in config["source_trf"]["files"].items()
    }
    expected["decisions"] = config["source_decisions"]["sha256"]
...
def assert_sources_unchanged(config: dict[str, Any], expected: dict[str, Any]) -> None:
    if snapshot_sources(config) != expected:
        raise TargetTRFError("Source TRF or self-annotation files changed during the run")


def implementation_hashes() -> dict[str, dict[str, Any]]:
    paths = sorted((PROJECT_ROOT / "code" / "trf_target").glob("*.py"))
    paths.extend(
        [
            PROJECT_ROOT / "code" / "common" / "qwen_client.py",
            PROJECT_ROOT / "scripts" / "run-target-trf.ps1",
...
def initialize_or_resume_run(
    ...
    *,
    ...
    current_source = snapshot_sources(config)
    compatibility = {
