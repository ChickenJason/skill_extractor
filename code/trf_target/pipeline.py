...
from trf_target.common import TargetTRFError, source_paths  # noqa: E402
...
def load_source_bundle(config: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(decisions) != config["source_decisions"]["expected_count"]:
        raise TargetTRFError("Source decisions must contain exactly 326 records")
...
def validate_independent_targets(value: Any) -> list[dict[str, Any]]:
    ...
    for position, item in enumerate(value):
        if not isinstance(item, dict) or set(item) != {"idx", "sentence"}:
            raise TargetTRFError(
                f"Target record {position} must contain exactly idx and sentence"
            )
    ...
    return sorted(records, key=lambda record: record["idx"])
...
def build_targets(
    ...
    if mode == "independent":
        ...
        records = [
            {"idx": item["idx"], "sentence": item["sentence"]}
...
def retrieve_demonstrations(
    ...
    *,
    ...
    for demo in demonstrations:
        if leave_one_out and demo["idx"] == target["idx"]:
            continue
    ...
    return {
        ...
        and any(demo["idx"] == target["idx"] for demo in demonstrations),
        "ranking_contract": {
