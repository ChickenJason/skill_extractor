"""Run one collaborative-reflection expert over frozen R0 inputs."""
...
def _latest(records: list[dict[str, Any]], branch: str) -> dict[int, dict[str, Any]]:
    ...
    for position, record in enumerate(records, start=1):
        if record.get("schema_version") != "collaborative-reflection-raw-v1" or record.get("branch") != branch:
            raise ReflectionError(f"Invalid {branch} raw record at line {position}")
        idx = record.get("idx")
        if type(idx) is not int:
            raise ReflectionError(f"Invalid {branch} raw idx at line {position}")
        identity = (record.get("dataset_id"), record.get("record_id"), record.get("source_sha256"), record.get("sentence"))
...
def _compatibility(
    config_path: Path, parent_root: Path, run_id: str, branch: str, paths: dict[str, Path]
) -> dict[str, Any]:
    return {
...
def _initialize(
    ...
    if paths["manifest"].exists():
        if not resume:
            raise ReflectionError(f"{branch} reflection manifest already exists")
        manifest = load_json(paths["manifest"])
        if manifest.get("compatibility") != compatibility:
            raise ReflectionError(f"{branch} reflection resume compatibility mismatch")
        if manifest.get("implementation") != implementation:
            raise ReflectionError(f"{branch} reflection implementation changed")
        ...
        manifest = {
            "schema_version": 1,
            "pipeline_version": f"{branch}-reflection-v1",
...
def _make_client(config: dict[str, Any]) -> tuple[QwenClient, dict[str, Any]]:
    ...
    return QwenClient(api_key=provider["api_key"], settings=settings), expanded
...
def run_branch(
    *,
    ...
    for idx in sorted(by_idx):
        ...
        from second_layer.collaborative_reflection.pipeline import (  # local to avoid duplicated online state
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"run_id": result["run_id"], "branch": args.branch, "status": result["status"]}, ensure_ascii=False))
        return 0
    except Exception as error:
        print(f"RunReflectionBranch failed: {error}", file=sys.stderr)
        return 1
