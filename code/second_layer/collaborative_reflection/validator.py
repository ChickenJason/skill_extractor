"""Read-only deterministic reconstruction of a completed collaborative-reflection run."""
...
def validate(config_path: Path, run_id: str) -> dict[str, Any]:
    ...
    paths = reflection_paths(config, run_id)
    manifest = load_json(paths.manifest)
    if manifest.get("status") != "completed":
        raise ReflectionError("Only completed collaborative-reflection runs can pass validation")
    ...
    if set(manifest.get("stages", {})) != set(STAGES) or any(
        ...
        raise ReflectionError("Every collaborative-reflection stage must be completed")
    ...
    if Path(manifest["compatibility"]["config"]["path"]).resolve() != config_path.resolve():
        raise ReflectionError("Validator was given a different reflection config")
    ...
    if any(branches[name]["status"] != "completed" for name in BRANCHES):
        raise ReflectionError("Both reflection branches must be completed")
    validation_results = {
    ...
    if not isinstance(processes, dict) or set(processes) != set(BRANCHES):
        raise ReflectionError("Reflection process audit is incomplete")
    if any(processes[name].get("exit_code") != 0 for name in BRANCHES):
        raise ReflectionError("A reflection process did not exit successfully")
    if not (
        processes["trf"]["started_at"] <= processes["exemplar"]["ended_at"]
        and processes["exemplar"]["started_at"] <= processes["trf"]["ended_at"]
    ):
        raise ReflectionError("Reflection branch process intervals do not overlap")
    ...
    _equal(actual_context, expected_context, "assembled collaborative-reflection context")
...
def main() -> int:
    ...
    try:
        ...
        print(f"ValidateCollaborativeReflection failed: {error}", file=sys.stderr)
        return 1
