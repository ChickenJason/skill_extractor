"""Read-only deterministic validator for one JSON-Schema reflection branch."""
...
from second_layer.collaborative_reflection_json_schema.branch_runner import (  # noqa: E402
    _branch_paths,
    _compatibility,
    _latest,
    _output_hashes,
...
def _validate_raw(
    ...
    for key in ("dataset_id", "record_id", "source_sha256", "idx", "sentence"):
        _equal(raw.get(key), record.get(key), f"{branch} raw {key}")
    _equal(raw.get("prompt_sha256"), prompt["prompt_sha256"], f"{branch} prompt hash")
    _equal(
        raw.get("response_format_sha256"), prompt["response_format_sha256"],
        f"{branch} response format hash",
    )
    _equal(
        raw.get("request_contract_sha256"), prompt["request_contract_sha256"],
        f"{branch} initial request contract hash",
    )
    ...
    for position, attempt in enumerate(attempts, start=1):
        ...
        _equal(attempt.get("kind"), "initial" if position == 1 else "repair", f"{branch} attempt kind")
        if position == 1:
            ...
            messages = build_repair_messages(
                prompt, branch, prior_error, prior_content, chat["max_reason_characters"]
            )
        _equal(
            attempt.get("request_sha256"),
            request_sha256(messages, prompt["response_format"]),
            f"{branch} request hash",
        )
...
def validate_branch(
    *, config_path: Path, parent_root: Path, run_id: str, branch: str,
    ...
    if manifest.get("status") != "completed":
        raise ReflectionError(f"Only completed {branch} JSON-Schema runs can pass validation")
    _equal(manifest.get("protocol_version"), PROTOCOL_VERSION, f"{branch} protocol")
    ...
    expected_prompts = [
        build_prompt(
            item, branch, config["chat"]["max_prompt_characters"],
    ...
    for raw in raw_records:
        ...
        if idx not in inputs_by_idx:
            raise ReflectionError(f"{branch} raw contains unknown idx={idx}")
    ...
    return {
        "schema_version": "collaborative-reflection-json-schema-branch-validation-v2",
