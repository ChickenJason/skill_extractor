"""Reflection-only Qwen client that sends strict JSON Schema response formats."""
...
def validate_response_format(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != {"type", "json_schema"}:
        raise ValueError("Reflection response_format must contain type and json_schema")
    schema = value.get("json_schema")
    if value.get("type") != "json_schema" or not isinstance(schema, dict):
        raise ValueError("Reflection response_format must use json_schema")
    if set(schema) != {"name", "strict", "schema"} or schema.get("strict") is not True:
        raise ValueError("Reflection JSON Schema must be named and strict")
    root = schema.get("schema")
    if not isinstance(root, dict) or root.get("type") != "object" or root.get("additionalProperties") is not False:
        raise ValueError("Reflection JSON Schema root must be a closed object")
    return copy.deepcopy(value)


class JsonSchemaQwenClient(QwenClient):
    """Keep the shared client byte-stable while adding a reflection-only request surface."""
    ...
    def chat(
        ...
        *,
        ...
        response, attempts = self._retry(  # type: ignore[attr-defined]
