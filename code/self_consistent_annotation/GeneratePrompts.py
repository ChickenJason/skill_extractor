...
def select_output_path(config: dict[str, Any], limit: int | None) -> Path:
    output_key = "preview_output" if limit is not None else "output"
    if output_key not in config:
        raise KeyError(f"Missing config field: {output_key}")
    return resolve_project_path(config[output_key])
...
def main() -> None:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "config" / "prompt.json",
    )
    ...
    prompt_path = resolve_project_path(config["prompt_file"])
    ...
    output_path = select_output_path(config, args.limit)
    prompts = generate_prompts(records, template, config["version"])
