...
from common.contracts import record_identity, record_source_sha256  # noqa: E402
from trf.target.common import TargetTRFError, latest_records  # noqa: E402
...
def load_embedding_records(
    ...
    for idx, record in latest.items():
        ...
        if record_identity(record, "embedding") != record_identity(item, "source"):
            raise TargetTRFError(f"Embedding identity mismatch for idx={idx}")
        if record_source_sha256(record, "embedding") != record_source_sha256(
            item, "source"
        ):
            raise TargetTRFError(f"Embedding source hash mismatch for idx={idx}")
    ...
    *,
    ...
    for item in items:
        ...
        if source and source["sentence_sha256"] == sentence_hash(item["sentence"]):
            record = {
                "schema_version": "embedding-record-v1",
...
def run_two_turn_extraction(
    ...
    *,
    ...
    for target in pending:
        ...
        record: dict[str, Any] = {
            "schema_version": "trf-raw-response-v1",
