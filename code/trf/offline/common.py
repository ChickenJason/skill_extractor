...
def load_trf_config(
    path: Path, demonstrations: str | Path | None = None
) -> dict[str, Any]:
    root_config = load_json(path)
    if root_config.get("schema_version") != 1 or root_config.get("module") != "trf":
        raise TRFError("config/trf.json must be a schema_version 1 TRF module config")
    config = root_config.get("offline")
    if not isinstance(config, dict):
        raise TRFError("TRF module config must define offline settings")
    config = json.loads(json.dumps(config))
    config["output"] = {"runs_root": root_config["output"]["runs_root"]}
    if config.get("schema_version") != 1:
        raise TRFError("TRF offline schema_version must equal 1")
    if config.get("pipeline_version") != "trf-offline-v2":
        raise TRFError("Unsupported TRF pipeline_version")
    source = config.get("source")
    if not isinstance(source, dict):
        raise TRFError("TRF offline config must define source settings")
    selected = demonstrations or source.get("demonstrations")
    if not isinstance(selected, (str, Path)) or not str(selected).strip():
        raise TRFError("Provide a neutral demonstration-set manifest")
    source["demonstrations"] = str(
        resolve_project_path(PROJECT_ROOT, selected).resolve()
    )
    return config
...
def source_files(config: dict[str, Any]) -> SourceFiles:
    manifest = resolve_project_path(
        PROJECT_ROOT, config["source"]["demonstrations"]
    ).resolve()
    bundle = load_demonstration_set(manifest)
    return SourceFiles(
        root=manifest.parent,
        manifest=manifest,
        records=bundle["records_path"],
        audit=bundle["audit_path"],
    )
...
def snapshot_source(config: dict[str, Any]) -> dict[str, Any]:
    ...
    bundle = load_demonstration_set(files.manifest)
    ...
    return {
        ...
        "files": {
            ...
            "records": {
                "path": str(files.records),
                "sha256": sha256_file(files.records),
                "bytes": files.records.stat().st_size,
            },
            "audit": {
                "path": str(files.audit),
                "sha256": sha256_file(files.audit),
                "bytes": files.audit.stat().st_size,
            },
        },
    }
...
def implementation_hashes() -> dict[str, dict[str, Any]]:
    files = sorted(
        (PROJECT_ROOT / "code" / "trf" / "offline").glob("*.py"),
