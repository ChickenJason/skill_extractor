...
def build_corpus_records(
    ...
    for decision in decisions:
        ...
        if not isinstance(dataset_id, str) or not isinstance(record_id, str):
            raise TRFError(f"idx={idx} has no module-neutral record identity")
        if not isinstance(source_sha256, str) or len(source_sha256) != 64:
            raise TRFError(f"idx={idx} has no source SHA256")
        ...
        if status == "accepted":
            ...
            corpus.append(
                {
                    "schema_version": "trf-corpus-record-v1",
...
def write_corpus(config: dict[str, Any], run_root: Any) -> dict[str, Any]:
    decisions = read_jsonl(source_files(config).records)
