"""Run the TRF-aware instance discriminator without modifying upstream artifacts."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--target-trf-run-id", required=True)
...
def run(
    ...
    *,
    ...
    bundle = load_source_bundle(config, args.target_trf_run_id, args.limit)
    descriptor = target_descriptor(bundle["targets"], args.limit)
    paths = initialize_or_resume_run(
        config_path,
        config,
        args.target_trf_run_id,
        args.run_id,
        descriptor,
        list(sys.argv),
        resume=args.resume,
