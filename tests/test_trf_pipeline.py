...
from trf.AssignPseudoTRFs import (  # noqa: E402
...
from trf.BuildTRFCorpus import build_corpus_records, validate_span  # noqa: E402
from trf.ExtractTRFCandidates import (  # noqa: E402
    _class_tokens,
...
from tests.trf_test_fixtures import prepare_v06_trf_config  # noqa: E402
...
class TRFSourceBindingTests(unittest.TestCase):
    def test_self_annotation_id_binds_input_without_changing_template(self) -> None:
        template_path = PROJECT_ROOT / "config" / "trf.json"
        template = load_json(template_path)
        self.assertNotIn("run_id", template["source"])
        self.assertNotIn("run_root", template["source"])
        with self.assertRaisesRegex(TRFError, "self_annotator_run_id"):
            load_trf_config(template_path)

        bound = bind_self_annotation_run(template, "self-source-0831")
        self.assertEqual(bound["source"]["run_id"], "self-source-0831")
        self.assertEqual(
            Path(bound["source"]["run_root"]),
            PROJECT_ROOT / "output" / "runs" / "self-source-0831",
        )
        self.assertNotIn("run_id", template["source"])

    def test_self_annotation_id_rejects_path_traversal(self) -> None:
        template = load_json(PROJECT_ROOT / "config" / "trf.json")
        with self.assertRaises(ValueError):
            bind_self_annotation_run(template, "../outside")


class TRFCorpusTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.decisions = read_jsonl(source_files(CONFIG).decisions)
...
class TRFTokenizerAndStatisticsTests(unittest.TestCase):
    ...
    def test_real_candidate_banks_have_fixed_top_twenty(self) -> None:
        corpus, _, _ = build_corpus_records(
            read_jsonl(source_files(CONFIG).decisions), CONFIG["source"]["expected_counts"]
        )
...
class TRFBertAlgorithmTests(unittest.TestCase):
    ...
    def test_offline_resolution_does_not_call_download_client(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("trf.AssignPseudoTRFs._repo_cache_directory", return_value=cache):
...
def _real_bert_smoke_ready() -> bool:
    if sys.version_info[:2] != (3, 10):
        return False
    snapshot = _repo_cache_directory(CONFIG["model"]["name"]) / "snapshots" / CONFIG["model"][
        "revision"
    ]
