...
from self_consistent_annotation.GeneratePrompts import (  # noqa: E402
    generate_prompts,
...
from self_consistent_annotation.SelectAnnotations import select_annotations  # noqa: E402
from self_consistent_annotation.SpanXMLCConsensus import (  # noqa: E402
    aggregate_records,
)


AGGREGATION = load_json(PROJECT_ROOT / "config" / "pipeline.json")["aggregation"]
...
class PromptAndPathTests(unittest.TestCase):
    def setUp(self) -> None:
        self.config = load_json(PROJECT_ROOT / "config" / "prompt.json")
        self.template = load_prompt_template(PROJECT_ROOT / self.config["prompt_file"])

    def test_prompt_contract_is_unchanged(self) -> None:
        self.assertEqual(self.config["version"], "skill-inline-annotation-v4.1")
    ...
    def test_prompt_generation_json_quotes_sentence(self) -> None:
        ...
        prompts = generate_prompts(records, self.template, self.config["version"])
    ...
    def test_dataset_hash_and_external_working_directory(self) -> None:
        ...
        script = PROJECT_ROOT / "code" / "self_consistent_annotation" / "GeneratePrompts.py"
...
class OfflineReplayTests(unittest.TestCase):
    def test_reaggregation_is_offline_immutable_and_records_hashes(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = load_json(PROJECT_ROOT / "config" / "pipeline.json")
