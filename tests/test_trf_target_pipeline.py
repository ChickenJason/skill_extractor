...
from trf_target.RunTargetTRF import run  # noqa: E402
from trf_target.ValidateTargetTRFRun import validate  # noqa: E402
from trf_target.common import TargetTRFError  # noqa: E402
...
class TargetTRFTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        create_fake_completed_offline(
            offline_path, SELF_RUN_ID, cls.offline_run_id
        )
    ...
    def test_template_is_rejected_by_direct_target_runner(self) -> None:
        args = self._args(PROJECT_ROOT / "config" / "trf_target.json")
        with self.assertRaisesRegex(TargetTRFError, "full-run template"):
            run(args)
    ...
    def test_formal_index_mismatch_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            copied = root / self.offline_run_id
    ...
    def _workspace(self, directory: str, targets: list[dict[str, Any]] | None = None):
        ...
        config_path = root / "trf_target.json"
    ...
    def test_independent_input_is_strict(self) -> None:
        self.assertEqual(
            validate_independent_targets([{"idx": 2, "sentence": "x"}]),
            [{"idx": 2, "sentence": "x"}],
        )
        invalid = [
            [{"idx": 1, "sentence": "x", "gold": ["Skill"]}],
            [{"idx": 1, "sentence": " "}],
    ...
    def test_prompt_and_strict_open_trf_parsing(self) -> None:
        ...
        target = {"idx": 7, "sentence": 'target "quoted"'}
    ...
    def test_mock_independent_integration_and_validator(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            manifest = load_json(runs / "test-run" / "manifest.json")
            self.assertEqual(manifest["status"], "completed")
            self.assertTrue(manifest["network_called"])
            parsed = read_jsonl(runs / "test-run" / "parsed" / "records.jsonl")
            self.assertEqual(len(parsed), 2)
            self.assertEqual(len(parsed[0]["trfs"]), 2)
            retrieval = read_jsonl(runs / "test-run" / "retrieval" / "records.jsonl")
    ...
    def test_partial_resume_and_explicit_failed_retry(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            initial_manifest = load_json(runs / "test-run" / "manifest.json")
            ...
            raw = read_jsonl(runs / "test-run" / "raw" / "responses.jsonl")
            ...
            self.assertEqual(
                len(read_jsonl(runs / "test-run" / "raw" / "responses.jsonl")),
                calls_after_failure,
            )
            ...
            latest = {
                item["idx"]: item
                for item in read_jsonl(runs / "test-run" / "raw" / "responses.jsonl")
            }
    ...
    def test_embedding_reuse_requires_exact_target_set(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            reused_manifest = load_json(runs / "same-target" / "manifest.json")
            self.assertFalse(reused_manifest["network_called"])
            target_cache = read_jsonl(
                runs / "same-target" / "embeddings" / "targets.jsonl"
            )
            ...
            self.assertFalse(
                (runs / "changed-target" / "embeddings" / "targets.jsonl").exists()
            )
