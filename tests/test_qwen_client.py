...
from common.io_utils import MissingEnvironmentVariable  # noqa: E402
from common.qwen_client import QwenClient, QwenSettings  # noqa: E402


ASK_PATH = PROJECT_ROOT / "code" / "self_consistent_annotation" / "AskQwen.py"
...
class QwenTests(unittest.TestCase):
    ...
    def test_missing_credentials_fail_before_any_output(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = json.loads(
                (PROJECT_ROOT / "config" / "pipeline.json").read_text(
                    encoding="utf-8"
                )
            )
    ...
    def test_config_rejects_non_five_sampling_and_bad_anchor_contract(self) -> None:
        config = json.loads(
            (PROJECT_ROOT / "config" / "pipeline.json").read_text(encoding="utf-8")
        )
